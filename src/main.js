...
const WEAPONS = [
  ...
  /*
   * The multiplayer roster. rpm is seconds between shots, so it is 60 divided
   * by the rounds-per-minute figure; damage is set from shots-to-kill against
   * a hundred-health walker, which is how everything above is balanced.
   */
  {
    id: "mtar",
    name: "MTAR",
...
/*
 * What a class may be built from: every gun, less the knife and less the
 * wonder weapons. A ray gun is something the box gives you for surviving,
 * not something you turn up to a match already holding.
 */
const NOT_IN_CLASSES = new Set(["raygun", "raygun2", "jetgun"]);
const gunsOnly = WEAPONS.filter((w) => !w.melee && !NOT_IN_CLASSES.has(w.id));
